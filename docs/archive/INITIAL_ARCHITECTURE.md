> ⚠️ **Historical / superseded architecture — not runtime truth.**
> This is the original design document, written before implementation began. The team pivoted away from most of what it describes (Node.js/Fastify/microservices, Next.js SSR, BullMQ, MinIO, a Turborepo monorepo) to a simpler Python/FastAPI monolith with a deterministic asyncio worker — a decision already made and shipped by the time this note was added, not one made in the course of writing it. See [`ARCHITECTURE.md`](../../ARCHITECTURE.md) at the repo root for what is actually running today.
>
> Kept for historical reference only. **Do not use this document to guide implementation work** — a task like "add the missing BullMQ queue" or "port apps/api to Fastify" would be reverting a deliberate, already-shipped architectural decision, not filling a gap.

# ROXASE — Architecture Document (Original / Historical)
